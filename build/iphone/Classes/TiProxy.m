...
//Rare exceptions to indicate a bug in the _monsto code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_monsto = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_monsto]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.monsto.module"
